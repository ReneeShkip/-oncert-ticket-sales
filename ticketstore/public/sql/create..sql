...
SELECT bt.ID, b.title, b.year, a.first_name, a.last_name, bt.price, b.cover
FROM
    book_type bt
...
WHERE
    b.id IN (
        ...
        FROM book_genre bg
        WHERE
            bg.genre_id IN (20, 21)
        GROUP BY
            bg.book_id
        HAVING
            COUNT(DISTINCT bg.genre_id) = 2
    );

CREATE OR REPLACE VIEW thriller_books AS
SELECT b.id, b.title, b.author, b.year, b.annotation, bt.price
FROM
    books b
